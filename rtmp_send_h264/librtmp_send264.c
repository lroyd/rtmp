 ...
 * @成功则返回 1 , 失败则返回0
 */
int ReadFirstNaluFromBuf(NaluUnit *_pNalu,int (*read_buffer)(uint8_t *buf, int buf_size)) 
{
...
gotnal_head:
		...
		{  
			...
			{  
				...
				{
					_pNalu->size = (naltail_pos - 3) - nalhead_pos;
					break;
				}
				else
				{
					...
					{	
						_pNalu->size = (naltail_pos - 4) - nalhead_pos;
						break;
					}
				}
			}  
		...
		_pNalu->type	= m_pFileBuf[nalhead_pos]&0x1f; 
		memcpy(m_pFileBuf_tmp, m_pFileBuf + nalhead_pos, _pNalu->size);
		_pNalu->data	= m_pFileBuf_tmp;
 ...
 * @成功则返回 1 , 失败则返回0
 */
int ReadOneNaluFromBuf(NaluUnit *_pNalu,int (*read_buffer)(uint8_t *buf, int buf_size))  
{    
	...
	while(1)
	{
		...
		{  
			...
			gotnal:	
				...
				{
					_pNalu->size = _pNalu->size + naltail_pos - nalustart;
					if (_pNalu->size > BUFFER_SIZE)
					{
						m_pFileBuf_tmp_old = m_pFileBuf_tmp;	//// save pointer in case realloc fails
						if ((m_pFileBuf_tmp = (unsigned char*)realloc(m_pFileBuf_tmp,_pNalu->size)) ==  NULL )
						{
							free( m_pFileBuf_tmp_old );  // free original block
							return FALSE;
						}
					}
					memcpy(m_pFileBuf_tmp + _pNalu->size + nalustart - naltail_pos, m_pFileBuf, naltail_pos - nalustart);
					_pNalu->data	= m_pFileBuf_tmp;
					nalhead_pos	= naltail_pos;
					return TRUE;
				}
				...
				{  
					_pNalu->type	= m_pFileBuf[nalhead_pos]&0x1f; 
					_pNalu->size	= naltail_pos - nalhead_pos - nalustart;
					if (_pNalu->type == 0x06)
					{
						nalhead_pos = naltail_pos;
						continue;
		...
		{
			_pNalu->size = BUFFER_SIZE - nalhead_pos;
			_pNalu->type = m_pFileBuf[nalhead_pos]&0x1f; 
			memcpy(m_pFileBuf_tmp, m_pFileBuf + nalhead_pos, _pNalu->size);
			if ((ret = read_buffer(m_pFileBuf,m_nFileBufSize)) < BUFFER_SIZE)
			{
				memcpy(m_pFileBuf_tmp + _pNalu->size,m_pFileBuf,ret);
				_pNalu->size	= _pNalu->size+ret;
				_pNalu->data	= m_pFileBuf_tmp;
		...
		if (nalhead_pos == GOT_A_NAL_CROSS_BUFFER || nalhead_pos == GOT_A_NAL_INCLUDE_A_BUFFER)
		{
			_pNalu->size = BUFFER_SIZE + _pNalu->size;
			...
			memcpy(m_pFileBuf_tmp + _pNalu->size - BUFFER_SIZE, m_pFileBuf, BUFFER_SIZE);
